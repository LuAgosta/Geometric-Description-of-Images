...
template <class T>
FixedImage<T>::FixedImage(PatchSize size)
...
template <class T>
FixedImage<T>::FixedImage(PatchSize size, uint number_of_channels)
...
template <class T>
FixedImage<T>::FixedImage(PatchSize size, T default_value)
 : _size_x(size.size_x), _size_y(size.size_y), _number_of_channels(1)
{
	init(size.size_x, size.size_y, 1);
	fill_internal(default_value);
...
template <class T>
FixedImage<T>::FixedImage(PatchSize size, uint number_of_channels, T default_value)
 : _size_x(size.size_x), _size_y(size.size_y), _number_of_channels(number_of_channels)
{
	init(size.size_x, size.size_y, number_of_channels);
	fill_internal(default_value);
...
template <class T>
PatchSize FixedImage<T>::get_size() const
{
	return PatchSize(_size_x, _size_y);
}
...
template <class T> /*Rect with 1 channel */
void FixedImage<T>::draw_rect(uint x, uint y, uint Width, uint Height, T value)
{
	for(uint i=x; i< x + Width + 1 ; i ++ ){
		for (uint j=y ; j< y + Height + 1 ; j ++){
			_data[get_index(i ,j ,0)]  = value ;
		}
	}
} /*Lucie*/


template <class T>  /*Rect with 3 channels but only one is used*/
void FixedImage<T>::draw_rect(uint x, uint y, uint Width, uint Height, uint channel, T value)
{
	for(uint i=x; i< x + Width + 1 ; i ++ ){
		for (uint j=y ; j< y + Height + 1 ; j ++){
			_data[get_index(i ,j ,channel)]  = value ;
		}
	}
} /*Lucie*/
...
template <class T>
Image<T>::Image(PatchSize size, uint number_of_channels)
...
template <class T>
Image<T>::Image(PatchSize size, T default_value)
...
template <class T>
Image<T>::Image(PatchSize size, uint number_of_channels, T default_value)
...
template <class T>
void Image<T>::draw_rect(uint x, uint y, uint Width, uint Height, T value)
{
	FixedImage<T>::draw_rect(x, y, Width, Height, value) ; 
} 


template <class T>
void Image<T>::draw_rect(uint x, uint y, uint Width, uint Height, uint channel, T value)
{
	FixedImage<T>::draw_rect(x, y, Width, Height, channel, value) ; 
} 