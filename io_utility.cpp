...
//TODO: Verificar lo que decia Vadim sobre si no estoy haciendo el estandar al 
//escribir y leer .flo
Image<float> IOUtility::read_flow_image(const string &name)
{
	int width, height, channels;
   ...
   if (! image_data) {
      fprintf(stderr, "read_flow_image: cannot read %s\n", name.data());
      exit(1);
   } 
   if (channels!= 2){
   	  fprintf(stderr, "read_flow_image: Incorrect numer of channesl:  %d\n", channels);
      exit(1);
   }

	Image<float> image(width, height, (uint)2);
...
void IOUtility::write_flow_image(const string &name, FixedImage<float> image)
{
	int width = image.get_size_x();
	int height = image.get_size_y();

	float *image_data = new float[width * height * 2];
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			...
			int index = y*width + x;
			int index2 = width*height + y*width + x;
			image_data[index]  = image(x, y, 0);
			image_data[index2] = image(x, y, 1);
		}
	}

	iio_save_image_float_split(const_cast<char* >(name.data()), image_data, width, height, 2);
...
////////////ROBERTO//////////////////////////////////////////////////////
void IOUtility::write_vectorial_image(const string &name, FixedImage<float> image)
{
	int width = image.get_size_x();
	int height = image.get_size_y();
	int n_channels = image.get_number_of_channels();

	float *image_data = new float[width * height * n_channels];

	// copy from Image<float> to image_data
 for (int i = 0; i < n_channels; i++){
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				image_data[y*width + x + i*width*height] = image(x, y, i);
			}
		}
}

	iio_save_image_float_split(const_cast<char* >(name.data()), image_data, width, height, n_channels);

}

void IOUtility::write_vectorial_image(const string &name, FixedImage<uint8_t> image)
{
	int width = image.get_size_x();
	int height = image.get_size_y();
	int n_channels = image.get_number_of_channels();
...
{
	PatchSize size = probabilities.get_size();
	Image<float> result(size.size_x, size.size_y, 0.0f);
...
Image<float> IOUtility::rgb_to_lab(FixedImage<float> image)
{
	if (image.get_number_of_channels() != 3) {
		printf("number of channels of image : %d \n" , image.get_number_of_channels()) ; /*Lucie*/
		return image;
	}
	...
	printf("number of channels of image_lab : %d \n" , image_lab.get_number_of_channels()) ; /*Lucie*/
